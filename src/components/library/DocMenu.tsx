/**
 * Document actions popover — rename, share, delete. Reached from the ⋮ on any
 * file-backed row or grid cell in the library.
 *
 * Delete confirms in place: the first tap arms the row ("Tap again to
 * delete"), the second commits. Rename swaps the action list for an input.
 */
...
export function DocMenu({
  ...
  /** Touch point on the ⋮, so the menu opens beside the row it belongs to. */
  anchor?: Anchor;
  doc: FilableDoc;
  /** The file on disk changed — rescan so every tab reflects it. */
  onChanged: () => void;
  onClose: () => void;
}) {
  ...
  const share = async () => {
    try {
      // Imported lazily: on a dev build made before expo-sharing was added,
      // a static import would crash the whole bundle at startup.
  ...
  const rename = () => {
    ...
    try {
      ...
      // entries keyed by the old uri follow the file to its new name
      useCollectionsStore
        .getState()
        .rename(doc.uri, { uri: file.uri, name: next });
      useRecentsStore.getState().rename(doc.uri, { uri: file.uri, name: next });
      // showToast(tr("library.docMenu.renamedToast", { name: next }));
      onChanged();
      onClose();
    } catch {

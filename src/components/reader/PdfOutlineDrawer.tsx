 ...
 *
 * Holds two lists: the document's own outline (embedded, or parsed off a
 * printed contents page) and the pages you've bookmarked. Either can be empty
 * — a PDF with no outline still opens this for its bookmarks, which is the
...
export function PdfOutlineDrawer({
  ...
  /** Bookmarked pages for this document, ascending. */
  bookmarks: number[];
  ...
  // With only one list there's nothing to choose, so the tab state is ignored
  // rather than trusted — it would otherwise strand you on an empty panel
  // after removing the last bookmark.
  ...
  /** Names the bookmarked page by the outline section it falls in. */
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Only worth a switch when there are two lists to switch between —
              otherwise the label above already says which one this is. */}
          {tabbed ? (
            <Box marginTop={13}>
              <Segmented
                items={[
        ...
        <ScrollView
          contentContainerStyle={{
            padding: 12,
            paddingTop: 8,
            // Clears the gesture bar / nav buttons; the footer that used to
            // provide this went away when bookmarks became a real list.

...
import { Backdrop, IconClose, IconSend, IconSpark, PText, Tap } from '@/components/proto';
...
const DOC_WORDS = [
  'edison', 'light', 'gas', 'lamp', 'pearl', 'electric', 'chapter', 'page', 'author', 'mean',
  'night', 'manhattan', 'summar', 'book', 'section', 'time', 'city', 'read', 'why', 'how',
];
...
export function LexiBubble({ onPress }: { onPress: () => void }) {
  ...
  return (
    <Tap onPress={onPress} scale={0.95} style={{ position: 'absolute', left: 18, bottom: 24 + insets.bottom, zIndex: 31 }}>
      <Box
        align="center"
        bg={t.card}
        borderColor={t.line}
  ...
  const send = () => {
    ...
    if (!isDoc) {
      ...
      if (rabbitCount.current >= 3) {
        ...
        replyIdx.current += 1;
        reply = { role: 'lexi', kind: 'normal', text: REPLY_POOL[replyIdx.current % REPLY_POOL.length] };
      }
    }

    setMessages([...messages, { role: 'user', kind: 'normal', text: q }, reply]);
    setInput('');
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Box align="center" bg={t.accentSoft} height={36} justify="center" rounded={12} width={36}>
        ...
        >
          {messages.map((m, i) => {
            ...
            return (
              <Box direction="row" justify={user ? 'end' : 'start'} key={i} paddingY={5}>
                <Box
                ...
                >
                  ...
                  {m.kind === 'drift' ? (
                    <Tap onPress={onClose} scale={0.95} style={{ alignSelf: 'flex-start' }}>
                  ...
                  {m.kind === 'recap' ? (
                    <Box bg={t.calmSoft} paddingX={9} paddingY={4} rounded={10} style={{ alignSelf: 'flex-start' }}>
        ...
        <Box
          paddingTop={12}
          paddingX={16}
          style={{ paddingBottom: 14 + insets.bottom, borderTopWidth: 1, borderTopColor: t.line }}
        >
          <Box align="center" bg={t.chip} direction="row" gap={9} paddingLeft={16} paddingRight={5} paddingY={5} rounded={24}>
            <TextInput
            ...
            <Tap onPress={send} scale={0.92}>
              <Box align="center" bg={t.accent} height={36} justify="center" rounded={18} width={36}>

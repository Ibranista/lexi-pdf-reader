/**
 * Search panel for real PDF documents.
 *
 * Queries the extracted WebView DOM and shows matching snippets with
 * highlighted keywords, page numbers, and a surrounding context extract.
 * The reflow WebView stays mounted in both view modes, so this works in
 * Page view too — tapping a result jumps the active view to that page.
 */
...
export function PdfSearchPanel({
  ...
  /** False while pages are still being extracted — matches may be partial. */
  indexed: boolean;
  onClose: () => void;
  /** `index` is the match's position in `results`, used to mark that exact hit. */
  ...
  // Explicit dismiss: persistTaps lets the tap land on the first press, but
  // the keyboard would otherwise linger over the destination page.
  ...
  return (
    <>
      ...
      >
        ...
        {/* Results list */}
        {/* persistTaps: without it the first tap only dismisses the keyboard
            and the result needs a second tap to open */}
        <ScrollView

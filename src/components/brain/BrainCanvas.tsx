/**
 * 3D "brain charging" visual — a 1:1 port of the three.js scene in
 * `extracted-design/LexiPDF Prototype.dc.html` (initBrain / tickBrain):
...
interface BrainCanvasProps {
  ...
  /** Milestone crossed after the intro animation settled. */
  readonly onMilestone?: (label: string) => void;
  /** Reports the animated fill percentage (integer). */
  readonly onPct?: (pct: number) => void;
  /** Books needed for a 100% full brain. */
...
/* ============ Brain shape (procedural gyri/sulci) ============ */

function brainShape(n: THREE.Vector3, out: THREE.Vector3): THREE.Vector3 {
  const M = THREE.MathUtils;
  // domain-warped ridged field -> worm-like gyri with sulci creases
  ...
  // deep longitudinal fissure between hemispheres
  ...
  // temporal lobe bulges on the lower sides
  const tl =
...
/** Soft radial dot texture for the spark sprites (no DOM canvas in RN). */
...
function createScene(gl: ExpoWebGLRenderingContext, ctl: Ctl) {
  ...
  // drifting idea motes inside the fluid
  const NPT = 240;
...
export function BrainCanvas({ booksDone, onMilestone, onPct, target }: BrainCanvasProps) {
  ...
  useEffect(() => {
    setPan(
      PanResponder.create({
        // the brain owns every touch that starts on it: drag in any direction
        // rotates (like the prototype) — page scrolling belongs to the content

...
export default function BrainScreen() {
  ...
  // sticky brain block's offset from the top of the page (status bar + header row)
  ...
  if (pct >= 100) frac = 1;
    ...
    frac =
      (i + (pct - MILESTONES[i]) / (MILESTONES[i + 1] - MILESTONES[i] || 1)) /
      4;
  }
  ...
  const nextLabel =
    nextMilestone === undefined
      ...
      : (() => {
          const need = Math.max(
            1,
            Math.ceil((nextMilestone / 100) * BRAIN_TARGET) - doneBooks,
          );
  ...
  return (
    <Box bg="#07080D" flex={1} style={{ paddingTop: insets.top }}>
      {/* radial-gradient(85% 46% at 50% 24%, #161C40 0%, #07080D 62%) */}
      <Svg
        height="100%"
      ...
      <ScrollView
        ...
        style={{ flex: 1 }}
      >
        {/* 3D brain — sticky: stays pinned while the sections below scroll behind it */}
        <Box height={308} style={{ overflow: "hidden" }}>
          {/* own copy of the page gradient, offset to line up with the fixed
              background; masked so the veil holds at 0.86 up top and fades to
              transparent toward the block's bottom edge — no hard line */}
          <Svg
            height={screenH}
            pointerEvents="none"
            preserveAspectRatio="none"
          ...
          >
            <Defs>
              ...
              <LinearGradient id="stickyFadeGrad" x1="0" x2="0" y1="0" y2="1">
                ...
                <Stop
                  offset={`${((headerBottom + 218) / screenH).toFixed(4)}`}
                  stopColor="#FFFFFF"
                  stopOpacity={0.86}
                />
                <Stop
                  offset={`${((headerBottom + 308) / screenH).toFixed(4)}`}
                  stopColor="#FFFFFF"
                  stopOpacity={0}
                />
          ...
          >
            <Svg height={290} width={290}>
              <Defs>
                {/* native react-native-svg drops rgba() alpha in stopColor — use stopOpacity */}
        ...
        {/* capacity — no negative overlap: the sticky brain block above is opaque */}
        <Box align="center" paddingTop={4} paddingX={28}>
          ...
          <Svg height={60} width={240}>
            <Defs>
              {/* linear-gradient(120deg, #AEB4FF 0%, #58C8FF 100%) */}
        ...
        <Box marginTop={22} marginX={20}>
          ...
          <Box>
            ...
            >
              {/* linear-gradient(90deg, #4C3FD4, #58C8FF) */}
              <Svg
                height={2}
                preserveAspectRatio="none"
                viewBox="0 0 100 2"
                ...
                return (
                  <Box align="center" flex={1} gap={7} key={m}>
                    <Box
                      ...
                      borderColor={
                        on ? "rgba(139,147,255,.6)" : "rgba(255,255,255,.12)"
                      }
                      borderWidth={1}
                      height={33}
                      justify="center"
                    ...
                    >
                      {on ? (
                        // linear-gradient(135deg, #4C3FD4, #3D7BFF) — drawn as a real
                        // circle so Android needs no view clipping
                        <Svg
                          height={31}
...
/**
 * Absolute-fill linear-gradient backdrop. Alpha is passed via `stopOpacity`
 * because native react-native-svg ignores the alpha channel of rgba() stopColors.
 */

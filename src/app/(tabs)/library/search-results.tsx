...
/** One searchable document, wherever the app knows it from. */
interface SearchDoc {
  uri: string;
  name: string;
  ext: string;
  ...
  /** Chip on the right: which part of the library this came from. */
  where: string;
}

/**
 * Ranks a title-prefix hit above a word-start hit above anything else, so
 * typing "tax" puts "Tax Return" above "Corporate tax appendix".
 */
...
/** Every query word has to appear somewhere in the name or format badge. */
function matches(doc: SearchDoc, tokens: string[]): boolean {
  const hay = `${doc.name} ${doc.ext}`.toLowerCase();
  return tokens.every((token) => hay.includes(token));
}

/** Last path segment of a folder uri, for folders the scan didn't name. */
...
export function SearchResults({
  ...
  /**
   * Everything searchable, deduped by uri. Scanned documents come first and
   * win, since they carry real size and date metadata; recents and filed
   * documents fill in files the scan can't reach — outside the picked folder,
   * or behind a permission that isn't granted this session.
   */

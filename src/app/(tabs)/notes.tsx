...
export default function NotesScreen() {
  ...
  const hlColor = (color: 'amber' | 'green') => (color === 'green' ? '#8FD9BE' : t.dark ? '#8A6A4F' : '#EFC9A0');
  ...
  return (
    <ProtoScreen>
      ...
      <ScrollView
        contentContainerStyle={{ padding: 20, paddingTop: 18, paddingBottom: 30 + insets.bottom, gap: 12 }}
        style={{ flex: 1 }}
      >
        {tab === 'bm' ? (
          <>
                ...
                <Tap key={p} onPress={() => jump(p)}>
                  <Box align="center" direction="row" gap={16}>
                    <Box align="end" width={44}>
                      <PText color={current ? t.accentText : t.faint} mono size={12} weight="600">
                        p. {p}
                      </PText>
                    </Box>
                    <Box
                      ...
                      style={
                        current
                          ? { shadowColor: t.accent, shadowOpacity: 0.4, shadowRadius: 4, elevation: 2 }
                          : undefined
                      }
                      width={8}
                    />
                    <Box flex={1}>
                      ...
                      >
                        ...
                        <Box flex={1}>
                          ...
                          <PText color={current ? t.accentText : t.sub} size={11.5} style={{ marginTop: 3 }}>
                            {current ? 'Current page' : `${Math.abs(p - page)} pages ${p < page ? 'back' : 'ahead'}`}
                          </PText>
                        </Box>
                      </Box>
                    </Box>
          ...
          noteList.map((n, i) => (
            <Tap key={`${n.p}-${i}`} onPress={() => jump(n.p)} scale={0.985}>
              <Card gap={10}>
                ...
                <Box style={{ borderLeftWidth: 3, borderLeftColor: hlColor(n.color), paddingLeft: 12 }}>
                  <PText lh={22} serif size={14}>
                    {n.text}
                  </PText>
                </Box>
                {n.note ? (
                  <Box bg={t.chip} direction="row" gap={8} paddingX={12} paddingY={10} rounded={10}>
                    <Box paddingTop={2}>
